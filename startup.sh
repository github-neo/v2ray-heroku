...
if [[ $TUNNEL_TOKEN ]]; then
    ...
    v2ray run -c /root/config.json & caddy run --config $CADDY_FILE & /root/cloudflared tunnel --no-autoupdate run --token $TUNNEL_TOKEN --protocol http2
else
    echo "Start V2Ray without cloudflared"
    v2ray run -c /root/config.json & caddy run --config $CADDY_FILE
fi
